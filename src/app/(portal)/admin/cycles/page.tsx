...
const TD: React.CSSProperties = { padding: '14px 20px', fontSize: '13px', color: '#475569' };
...
export default function CycleSettingsPage() {
  ...
  return (
    <div className="animate-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      {/* Header */}
      <div>
        <h1 style={{ fontSize: '24px', fontWeight: 700, color: '#0f172a', margin: '0 0 4px 0' }}>Cycle Settings</h1>
        <p style={{ fontSize: '14px', color: '#64748b', margin: 0 }}>{activeCycle?.name} schedule, demo windows, and admin exception handling.</p>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '700px' }}>
            ...
            <tbody>
              {DEFAULT_CYCLE_WINDOWS.map((window, i) => (
                <tr key={window.name} style={{ borderBottom: i < DEFAULT_CYCLE_WINDOWS.length - 1 ? '1px solid #e2e8f0' : 'none' }} className="hover:bg-slate-50 transition-colors">
                  <td style={{ ...TD, fontWeight: 500, color: '#0f172a' }}>{window.name}</td>
                  <td style={TD}>{formatDate(window.opensAt)}</td>
                  <td style={TD}>{formatDate(window.closesAt)}</td>
                  <td style={{ ...TD, color: '#64748b' }}>{window.action}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e2e8f0' }}>
          <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#0f172a', margin: '0 0 4px 0' }}>Admin Unlock Exceptions</h2>
          <p style={{ fontSize: '13px', color: '#64748b', margin: 0 }}>Locked goals require HR/Admin intervention before employee edits are re-enabled.</p>
        </div>
        {lockedSheets.length === 0 ? (
          <div style={{ padding: '48px 20px', textAlign: 'center' }}>
            <p style={{ fontSize: '14px', color: '#94a3b8', margin: 0 }}>No locked sheets are available for exception handling.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '700px' }}>
                ...
                {lockedSheets.map((sheet, index) => {
                  ...
                  return (
                    <tr key={sheet.id} style={{ borderBottom: index < lockedSheets.length - 1 ? '1px solid #e2e8f0' : 'none' }}>
                      <td style={TD}>
                        <p style={{ fontSize: '14px', fontWeight: 500, color: '#0f172a', margin: '0 0 2px 0' }}>{employee?.name || sheet.employeeId}</p>
                        <p style={{ fontSize: '12px', color: '#94a3b8', margin: 0 }}>{employee?.department}</p>
                      </td>
                      <td style={TD}>
                        <span style={{ display: 'inline-flex', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: 600, background: '#d1fae5', color: '#065f46' }}>Locked</span>
                      </td>
                      <td style={TD}>
                        <input
                          ...
                          style={{ width: '100%', height: '34px', padding: '0 12px', borderRadius: '6px', border: '1px solid #e2e8f0', background: '#f8fafc', fontSize: '13px', outline: 'none', fontFamily: 'inherit' }}
                        />
                      </td>
                      <td style={{ ...TD, textAlign: 'right' }}>
                        <button
                          onClick={() => unlockSheet(sheet.id)}
                          style={{ height: '32px', padding: '0 16px', borderRadius: '6px', border: 'none', background: '#f59e0b', fontSize: '12px', fontWeight: 600, color: '#fff', cursor: 'pointer' }}
                        >Unlock</button>
                      </td>
                    </tr>
                  );
